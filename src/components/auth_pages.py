...
def show_login_form():
    with st.form("login_form"):
        ...
        if st.secrets.get("DEV_MODE", False):
            if st.form_submit_button("🧪 Dev Login", use_container_width=True, type="secondary"):
                success, result = SessionManager.login("test@example.com", "dev-pass")
                if success:
                    st.success("Bypassing auth for testing...")
                    time.sleep(0.5)
                    st.rerun()
...
def show_signup_form():
    with st.form("signup_form"):
        ...
        # Dev Mode Bypass Button
        if st.secrets.get("DEV_MODE", False):
            if st.form_submit_button("🧪 Dev Sign Up", use_container_width=True, type="secondary"):
                success, response = st.session_state.auth_service.sign_up(
                    "test@example.com", "dev-pass", "Dev User"
                )
                if success:
                    st.session_state.authenticated = True
                    st.session_state.user = response
                    st.success("Bypassing signup for testing...")
                    time.sleep(0.5)
                    st.rerun()
