...
def show_analysis_form():
    ...
    report_source = st.radio(
        ...
        ["Upload PDF", "Paste Text", "Use Sample PDF"],
        index=0 if st.session_state.get("report_source") == "Upload PDF" else (1 if st.session_state.get("report_source") == "Paste Text" else 2),
        horizontal=True,
        key="report_source",
    )
...
def get_report_contents(report_source):
    if report_source == "Upload PDF":
        ...
        if uploaded_file:
            ...
            return pdf_contents
    elif report_source == "Paste Text":
        pasted_text = st.text_area(
            "Paste report text here",
            height=300,
            help="Copy the text from your report and paste it here if the PDF upload is not working correctly.",
        )
        if pasted_text:
            return pasted_text
        return None
